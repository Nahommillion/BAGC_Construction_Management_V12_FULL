...
def inject():
    return {"me":current_user(),"machine_types":MACHINE_TYPES,"material_categories":MATERIAL_CATEGORIES,"design_statuses":DESIGN_STATUSES,"today":dt.date.today().isoformat()}
...
def dashboard_data(pid=None):
    ...
    for p in projects:
        ...
        pe=c.execute("SELECT COALESCE(SUM(mp.present*mp.daily_rate+mp.overtime_hours*mp.overtime_rate),0) x FROM manpower mp WHERE mp.project_id=?",(p["id"],)).fetchone()["x"]
        ...
        out.append({"p":p,"income":money(inc),"expense":money(me+pe+se+other),"machine_expense":money(me),"manpower_expense":money(pe),"store_expense":money(se),"other_expense":money(other),"workers":workers,"machines":machines})
    c.close();return out
...
def daily(pid):
    ...
    if request.method=="POST":
        ...
        if request.form.get("section")=="boq":
            ...
            c.execute("INSERT INTO manpower(project_id,date,name,employment,position,present,daily_rate,overtime_hours,overtime_rate,notes,user_id) VALUES(?,?,?,?,?,?,?,?,?,?,?)",(pid,d,request.form["name"],request.form["employment"],request.form["position"],parse_float(request.form["present"]),parse_float(request.form["daily_rate"]),parse_float(request.form["overtime_hours"]),parse_float(request.form["overtime_rate"]),request.form.get("notes",""),current_user()["id"]))
...
def machinery(pid):
    ...
    if request.method=="POST":
        ...
        if action=="add":
            c.execute("INSERT INTO machines(project_id,machine_type,code,ownership,hourly_rate,expected_fuel) VALUES(?,?,?,?,?,?)",(pid,request.form["machine_type"],request.form["code"],request.form["ownership"],parse_float(request.form["hourly_rate"]),parse_float(request.form["expected_fuel"])))
...
def projects_admin():
    c=db()
    if request.method=="POST":
        try:c.execute("INSERT INTO projects(name,code,location,client,consultant,status,start_date,end_date) VALUES(?,?,?,?,?,?,?,?)",(request.form["name"],request.form["code"],request.form["location"],request.form.get("client",""),request.form.get("consultant",""),request.form.get("status","Active"),request.form.get("start_date",""),request.form.get("end_date","")));c.commit();flash("🏗️ Project created.","success")
